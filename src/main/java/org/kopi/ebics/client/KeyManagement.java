...
/**
 * Everything that has to do with key handling. If you have a totally new
 * account use <code>sendINI()</code> and <code>sendHIA()</code> to send you
 * newly created keys to the bank. Then wait until the bank activated your keys.
 * If you are migrating from FTAM. Just send HPB, your EBICS account should be
...
public class KeyManagement {
	...
	/**
	 * Constructs a new <code>KeyManagement</code> instance with a given ebics
	 * session
	 * 
	 * @param session
	 *            the ebics session
	...
	/**
	 * Sends the user's signature key (A005) to the bank. After successful
	 * operation the user is in state "initialized".
	 * 
	 * @param orderId
	 *            the order ID. Let it null to generate a random one.
	...
	/**
	 * Sends encryption and authentication keys to the bank. This order is only
	 * allowed for a new user at the bank side that has been created by copying
	 * the A005 key. The keys will be activated immediately after successful
	...
	public byte[] sendHPB(byte[] keyStore) throws IOException, GeneralSecurityException, EbicsException {
		...
		factory = new ByteArrayContentFactory(
				Utils.unzip(session.getUser().decrypt(response.getOrderData(), response.getTransactionKey())));
		orderData = new HPBResponseOrderDataElement(factory);
		orderData.build();
		this.configuration.getTraceManager().trace(orderData);
		...
		if (session.getBank().useCertificate()) {
			...
			session.getBank().setBankKeys(e002PubKey, x002PubKey);
			session.getBank().setDigests(KeyUtil.getKeyDigest(e002PubKey), KeyUtil.getKeyDigest(x002PubKey));
			keystoreManager.setCertificateEntry(session.getBankID() + "-E002",
					new ByteArrayInputStream(orderData.getBankE002Certificate()));
			keystoreManager.setCertificateEntry(session.getBankID() + "-X002",
					new ByteArrayInputStream(orderData.getBankX002Certificate()));
			keystoreManager.save(updateKeyStoreStream);
		} else {
			e002PubKey = keystoreManager.getPublicKey(new BigInteger(orderData.getBankE002PublicKeyExponent()),
					new BigInteger(orderData.getBankE002PublicKeyModulus()));
			x002PubKey = keystoreManager.getPublicKey(new BigInteger(orderData.getBankX002PublicKeyExponent()),
					new BigInteger(orderData.getBankX002PublicKeyModulus()));
			session.getBank().setBankKeys(e002PubKey, x002PubKey);
			session.getBank().setDigests(KeyUtil.getKeyDigest(e002PubKey), KeyUtil.getKeyDigest(x002PubKey));
			keystoreManager.save(updateKeyStoreStream);
		}
	...
	/**
	 * Sends the SPR order to the bank. After that you have to start over with
	 * sending INI and HIA.
	 * 
	 * @throws IOException
	 *             Communication exception
	 * @throws EbicsException
	 *             Error message generated by the bank.
	 */
